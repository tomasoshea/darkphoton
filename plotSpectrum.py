...
ax2.plot(datT[:,0], phiT, ls = '--', color=(1., 0.8, 0.), label = 'T plasmon m = 100 eV')

datT = loadtxt("data/Espectrum3.dat")	# m = 1e2 eV
phiT = datT[:,1] / np.nanmax(datT[:,1])	# normalise phi
ax2.plot(datT[:,0], phiT, ls = '--', color='magenta', label = 'T plasmon m = 1 keV')
...
fig2.savefig('plots/spectrum_eV2-1.jpg')
